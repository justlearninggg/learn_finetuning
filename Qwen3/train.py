...
def process_func(example):
    ...
    # 将response和prompt拼接,input_ids是已经经过tokenizer处理过后的数字序列了
...
train_ds = Dataset.from_pandas(train_df) #转成HF Dataset
...
args = TrainingArguments(
    ...
    eval_strategy="steps", # 评估策略（每 100 步评估一次）
    eval_steps=100,
    logging_steps=10, # 每10个step打印一个loss
    num_train_epochs=2,
    save_steps=400, # 每400个step保存一次模型
    learning_rate=1e-4, # 常见
    save_on_each_node=True, # 多卡训练时，每个节点都保存
    gradient_checkpointing=True, # 梯度检查点（显存换时间）
    report_to="swanlab", # 集成swanlab
    run_name="qwen3-1.7B",
)
